...
import type { DebouncedSearchSelectProps, DebouncedSearchSelectEmits, OptionItem } from './types';

// 定义组件属性
const props = withDefaults(defineProps<DebouncedSearchSelectProps>(), {
...
// 定义事件
const emit = defineEmits<DebouncedSearchSelectEmits>();
...
// 远程搜索方法
const debouncedRemoteMethod = (query: string) => {
  keyword.value = query;
  if (query !== '') {
...
// 暴露方法
defineExpose({
  refreshOptions: (query = '') => debouncedFetch(query),
  clearOptions: () => {
    options.value = [];
  },
});
